...
float MidiParser::GetStart(int i) {
    if (i < 0)
        ...
        if (mCurParser == mGemParser) {
            ...
            MILO_NOTIFY("%s calling get_start outside of gem or note parser", Name());
        }
        return kHugeFloat;
    }
}
...
float MidiParser::GetEnd(int i) {
    if (i < 0)
        ...
        if (mCurParser == mGemParser) {
            ...
            MILO_NOTIFY("%s calling get_end outside of gem or note parser", Name());
        }
        return kHugeFloat;
    }
}
...
void MidiParser::SetGlobalVars(int startTick, int endTick, const DataNode &data) {
    ...
    *mpData = data;
    if (mCurParser == mTextParser) {
        MILO_ASSERT(data.Type() == kDataArray, 0x226);
        ...
        *mpVal = data;
        if (mCurParser == mGemParser) {
            ...
            if (lowestBit > 0x1000000) {
                MILO_NOTIFY("Bad gem, value 0x%x", gemval);
            } else {
                for (; i6 > 0; i6 >>= 1) {
                    if (gemval & i6)
                        break;
